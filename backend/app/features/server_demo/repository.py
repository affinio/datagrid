...
class ServerDemoRepository:
    ...
    async def commit_edits(self, request: ServerDemoCommitEditsRequest) -> ServerDemoCommitEditsResponse:
        ...
        return ServerDemoCommitEditsResponse(
            ...
            rejected=rejected,
            revision=revision,
            invalidation=self._build_history_invalidation("edit", affected_indexes),
        )

    async def resolve_fill_boundary(self, request: ServerDemoFillBoundaryRequest) -> ServerDemoFillBoundaryResponse:
        conditions = self._build_filter_conditions(request.projection.filter_model)
        stmt = self._build_filtered_row_query(conditions)
        stmt = stmt.order_by(*self._build_order_by(request.projection.sort_model))
