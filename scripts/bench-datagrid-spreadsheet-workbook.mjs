...
const BENCH_ITERATIONS = Number.parseInt(process.env.BENCH_SPREADSHEET_WORKBOOK_ITERATIONS ?? "25", 10)
const BENCH_ROW_COUNT = Number.parseInt(process.env.BENCH_SPREADSHEET_WORKBOOK_ROW_COUNT ?? "10000", 10)
const BENCH_PATCH_SIZE = Number.parseInt(process.env.BENCH_SPREADSHEET_WORKBOOK_PATCH_SIZE ?? "200", 10)
...
if (!Number.isInteger(BENCH_ITERATIONS) || BENCH_ITERATIONS <= 0) {
  throw new Error("BENCH_SPREADSHEET_WORKBOOK_ITERATIONS must be a positive integer.")
}
if (!Number.isInteger(BENCH_ROW_COUNT) || BENCH_ROW_COUNT <= 0) {
  throw new Error("BENCH_SPREADSHEET_WORKBOOK_ROW_COUNT must be a positive integer.")
}
if (!Number.isInteger(BENCH_PATCH_SIZE) || BENCH_PATCH_SIZE <= 0) {
  throw new Error("BENCH_SPREADSHEET_WORKBOOK_PATCH_SIZE must be a positive integer.")
}
...
async function loadSpreadsheetApi() {
  ...
  for (const candidate of candidates) {
    ...
    if (
      typeof module.createDataGridSpreadsheetSheetModel === "function"
      && typeof module.createDataGridSpreadsheetWorkbookModel === "function"
    ) {
      return {
...
function buildOrdersRows() {
  return Array.from({ length: BENCH_ROW_COUNT }, (_, rowIndex) => ({
    id: `order-${rowIndex + 1}`,
    cells: {
      amount: String((rowIndex % 25) + 1),
      tax: String((rowIndex % 5) + 1),
      total: "=[amount]@row + [tax]@row",
    },
  }))
}

function createHarness(api) {
  const amounts = Array.from({ length: BENCH_ROW_COUNT }, (_, rowIndex) => (rowIndex % 25) + 1)
  const ordersSheet = api.createDataGridSpreadsheetSheetModel({
    sheetId: "orders",
    sheetName: "Orders",
    columns: [
      { key: "amount" },
    ...
    rows: buildOrdersRows(),
    referenceParserOptions: {
      syntax: "smartsheet",
    },
  })

  const summarySheet = api.createDataGridSpreadsheetSheetModel({
    sheetId: "summary",
    sheetName: "Summary",
    columns: [{ key: "grandTotal" }],
    rows: [{
      id: "summary-1",
      cells: {
        grandTotal: "=SUM(TABLE('orders', 'total'))",
      },
    }],
  })

  const dashboardSheet = api.createDataGridSpreadsheetSheetModel({
    sheetId: "dashboard",
    sheetName: "Dashboard",
    columns: [{ key: "value" }],
    rows: [{
      id: "dashboard-1",
      cells: {
        value: "=SUM(TABLE('summary', 'grandTotal'))",
      },
    }],
  })

  const workbook = api.createDataGridSpreadsheetWorkbookModel({
    sheets: [
      { id: "orders", name: "Orders", sheetModel: ordersSheet, ownSheetModel: true },
      { id: "summary", name: "Summary", sheetModel: summarySheet, ownSheetModel: true },
      { id: "dashboard", name: "Dashboard", sheetModel: dashboardSheet, ownSheetModel: true },
    ],
  })
  ...
  return {
    ...
    sample(iteration) {
      ...
      for (let offset = 0; offset < BENCH_PATCH_SIZE; offset += 1) {
        ...
        amounts[rowIndex] = (amounts[rowIndex] % 1000) + 1
        patches.push({
          cell: {
            sheetId: "orders",
          ...
          rawInput: String(amounts[rowIndex]),
        })
      }
