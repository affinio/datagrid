...
| Touch/mobile scrolling | Partial | Native body scrolling, scroll-first touch policy, explicit touch handles, viewport-relative touch overscan, and retained render windows during smooth touch scroll are implemented. | Real-device matrix and hardware threshold review are still open. |
| Custom renderers during scroll | Partial | Touch active-scroll keeps visible custom cell/group renderers mounted and now reduces renderer churn for large smooth touch scroll windows. | Desktop custom-renderer frame budget and degradation policy are not isolated yet. |
