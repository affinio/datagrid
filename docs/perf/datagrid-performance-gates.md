...
- Browser-frame artifacts retain legacy `droppedPct` for compatibility and now expose `refreshAwareDroppedFramePct`/`refreshAwareDroppedFrames`, calculated from the configured `BENCH_BROWSER_REFRESH_RATE_HZ` (60 Hz by default). The refresh-aware metric counts missed refresh opportunities and is the basis for future calibrated 60/120 Hz UX budgets.
