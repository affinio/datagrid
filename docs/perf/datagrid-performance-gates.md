...
- Browser-frame artifacts retain legacy `droppedPct` for compatibility and expose `refreshAwareDroppedFramePct`/`refreshAwareDroppedFrames`, calculated from `BENCH_BROWSER_REFRESH_RATE_HZ` (60 Hz by default).
- The generic CI enterprise browser smoke gate allows up to `100%` refresh-aware dropped opportunities because shared runners can be heavily scheduled; dedicated `60hz`/`120hz` profiles keep the calibrated `35%`/`25%` hard limits.
- Calibrated profile entrypoints are `pnpm run bench:datagrid:enterprise:browser-frames:60hz:assert` and `pnpm run bench:datagrid:enterprise:browser-frames:120hz:assert`. They hard-fail the refresh-aware dropped opportunity rate at `35%` and `25%` respectively; the legacy percentage remains reported for compatibility.
...
- Datasource churn (range pull churn + invalidation pressure):
  - `PERF_BUDGET_TOTAL_MS=25000`, calibrated from the current Node 24 CI profile; elapsed remains reported alongside hard scenario and diagnostic budgets.
  - Scroll/filter/placeholder latency and diagnostic budgets remain hard failures and are the actionable datasource performance gates.
...
- Tree workload (deep hierarchy expand/filter/sort pressure):
  ...
  - Tree subtree toggle replacement uses one projection copy plus manual bounded shift/insert, avoiding temporary prefix/suffix arrays and spread argument limits; the remaining O(P) projection/index cost is measured by the group-depth workload.
...
- Vertical viewport virtualization maps measured native scroll extent to the logical row coordinate only when the native range is smaller than the logical range; identity mapping remains the default, and the mapping contract is covered by `verticalScrollMapping.contract.spec.ts`.
