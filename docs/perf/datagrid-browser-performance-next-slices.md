...
- Enterprise browser-frame artifacts now include `scrollFrameAttribution`, which summarizes slow writes, per-write max frame duration, long-task overlap, and mutation callback/mount counts for jump-scroll diagnostics. Opt-in `BENCH_BROWSER_OBSERVE_CHARACTER_DATA=true` adds text-node mutation counts for local diagnosis only; enabling it has observer overhead and should not be used for frame-budget comparisons.
...
- Desktop vertical retained row windows now shift only after the visible range reaches the final quarter of the effective overscan buffer, reducing periodic `stageWindowFlush` cadence during smooth vertical scroll while leaving coarse-pointer bounded chunk shifts unchanged.
- Rejected custom-renderer candidates: shallow nested native text normalization regressed frame p95 in `rendering-slow-custom-renderers`, and visual-slot renderer output caching broke the existing renderer lifecycle/remount contracts. Further custom-renderer work needs an explicit lifecycle/degradation design instead of transparent renderer result caching.
- Single-column client sorting now uses a numeric fast path when all non-null sort values are number-like and no custom comparator is configured, bounded integer numeric sorts use a bucket fast path when the value range is small enough, row-id tie-breaks avoid the generic comparator path for common numeric/text identities, and repeated column-menu sort selections skip no-op projection applies.
- `sort-only` visible-row refresh diagnostics now read `data-row-id` from recycled cell shells and choose the opposite sort direction when the requested direction is already active, so fixed-slot row recycling and no-op sort guards no longer turn completed runs into 2s benchmark timeouts. Main-thread spikes still come from synchronous full-table client sorting.
