...
- Single-column client sorting now uses a numeric fast path when all non-null sort values are number-like and no custom comparator is configured, row-id tie-breaks avoid the generic comparator path for common numeric/text identities, and repeated column-menu sort selections skip no-op projection applies.
