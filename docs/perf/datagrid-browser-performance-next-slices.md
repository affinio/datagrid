...
- Single-column client sorting now uses a numeric fast path when all non-null sort values are number-like and no custom comparator is configured, avoiding per-comparison comparator context allocation for common numeric columns.
