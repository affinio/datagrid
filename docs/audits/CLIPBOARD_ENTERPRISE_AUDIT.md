...
1. **Browser clipboard permission failures are mostly silent.**
   `useDataGridClipboardBridge.ts` catches write failures and still stores the in-memory payload. `readClipboardPayload` catches read failures and falls back to memory. This is pragmatic for tests and same-session copy/paste, but enterprise UX should tell users when system clipboard access was denied or when paste used stale in-memory data.
...
4. **Partial paste reporting is weak in the canonical app path.**
   Non-editable, group, and missing rows are skipped, and paste returns the number of updated rows rather than applied and blocked cell counts. The older `useDataGridClipboardMutations.ts` tracks applied/blocked cell counts, but the canonical `useDataGridAppClipboard.ts` does not surface that fidelity.

5. **Paste result semantics are row-count based, not cell-count based.**
   `applyClipboardEdits` returns updated row count. For enterprise UX and telemetry, paste should distinguish target cells, applied cells, skipped cells, blocked cells, materialized rows, and failed cells.
...
| Partial paste | Skips blocked cells/rows | Need applied/blocked cell counts and user feedback |
| Multi-range copy | Visuals track ranges; payload active-range-like | Need explicit policy and tests |
| Browser clipboard APIs | Uses `navigator.clipboard` with fallback | Need permission-denied messaging and secure-context expectations |
| Security restrictions | Catches denied reads/writes safely | Need clear UX and no stale fallback surprise |
...
- Clipboard write failure still reports copy success because the in-memory payload was stored.
- Clipboard read failure can paste stale in-memory data without telling the user.
...
- Partial paste does not report blocked cells in the canonical app path.
...
- No browser clipboard permission/a11y/mobile validation gate.
