...
3. **Sort/edit/context-menu frame cleanup**
   ...
   - Single-column local sort projection now avoids per-row sort-value array allocation and preserves direction-flip cache reuse.
   - Current interaction-frame artifact shows context-menu is not the active blocker; remaining deep cleanup should target deeper sort execution policy and inline-edit commit bursts.
