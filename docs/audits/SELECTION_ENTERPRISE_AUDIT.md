...
| Virtualization remount continuity | Logical model is suitable; vertical and right-pinned vertical remounts are browser-covered; horizontal selected-column remount is component-covered | Needs browser tests for horizontal remount, editors, and server placeholders after remount |
...
- Active/focus/edit ownership is specified as one state machine, but browser-level horizontal remount, editor remount, and server placeholder proof are still incomplete.
