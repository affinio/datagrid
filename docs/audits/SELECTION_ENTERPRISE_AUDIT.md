...
| --- | --- | --- |
| Active cell ownership | Documented across snapshot, anchor, focus runtime, and editing with focused contracts | Add editor remount and browser-level server placeholder tests |
...
| Virtualization remount continuity | Logical model is suitable; vertical, right-pinned vertical, and horizontal selected-cell remounts are browser-covered; horizontal selected-column remount and placeholder materialization selection handoff are component-covered | Needs browser tests for editors and server placeholders after remount |
...
- Active/focus/edit ownership is specified as one state machine, but editor remount and browser-level server placeholder proof are still incomplete.
...
2. Add e2e tests for server placeholders and editor remount state.
