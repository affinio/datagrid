...
| Body/header touch pan | Native scroll surface | Table-stage body touch pan stays native/passive; the table stage does not install a non-passive touchmove routing fallback. Explicit touch handles may still cancel their own handle gestures. |
| Header wheel / linked wheel | Stage scroll sync | Vertical wheel/overscroll is released to native behavior. Horizontal linked wheel may prevent default when translating the gesture into the center horizontal scroll owner. |
