...
- Status: Partially completed on 2026-05-18. The stage now tracks grid focus ownership across virtualized cell unmount/remount and restores the visible selection anchor after scroll idle, so keyboard navigation resumes from the remounted active selection. E2E coverage now proves vertical scroll-out/scroll-in remount preserves anchor class, overlay segment, fill handle, and keyboard focus, including a right-pinned selected cell remounting after vertical virtualization. Component coverage now proves horizontally virtualized selected columns restore anchor and fill-handle affordances when remounted, and placeholder row materialization moves the active selection anchor/fill affordance onto the created real row after inline edit commit. Editor remount, browser-level server placeholder replacement, and broader browser horizontal virtualization coverage remain planned.
