...
- Status: Partially completed on 2026-05-18. The stage now tracks grid focus ownership across virtualized cell unmount/remount and restores the visible selection anchor after scroll idle, so keyboard navigation resumes from the remounted active selection. E2E coverage now proves vertical scroll-out/scroll-in remount preserves anchor class, overlay segment, fill handle, and keyboard focus, including a right-pinned selected cell remounting after vertical virtualization, and proves horizontally virtualized editable selected cells restore anchor, fill handle, and overlay after remount. Component coverage also proves horizontally virtualized selected columns restore anchor/fill-handle affordances when remounted, and placeholder row materialization moves the active selection anchor/fill affordance onto the created real row after inline edit commit. Editor remount and browser-level server placeholder replacement coverage remain planned.
