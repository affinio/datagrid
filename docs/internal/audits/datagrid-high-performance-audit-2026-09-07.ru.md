...
- **Исправление A:** выполнено для host command exceptions: обработчик возвращает тот же `requestId` с `snapshot.error` и `loading=false`, сохраняя существующий update protocol shape; proxy pending lifecycle, worker termination и transport-level `messageerror` остаются отдельными cases.
