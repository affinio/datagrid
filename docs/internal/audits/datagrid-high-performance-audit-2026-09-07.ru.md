...
- **API sub-slice:** согласованный typed accessor `readCell({ rowKey, columnKey, valueField })` различает `missing`, `null` и `value`, не материализуя соседние cells; bootstrap delegation и sparse value/missing contract покрыты focused test. Export/drilldown adapters и `maxBytes` остаются следующими slices.
