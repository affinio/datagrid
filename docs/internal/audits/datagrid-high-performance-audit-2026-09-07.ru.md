...
- **DoD:** host exception и uncloneable row reply покрыты worker regression tests; при неудаче structured clone отправляется metadata-only terminal update с тем же `requestId`; lost response покрыт transport timeout contract (`inflight` возвращается к `0`, `timedOut` увеличивается). Worker termination и slow worker + rapid scroll/patch bursts остаются отдельными validation cases.
