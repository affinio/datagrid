...
- **DoD:** artifact показывает состав и стоимость documented capability entrypoints и повторяемый startup baseline; regression gate для базового consumer и consumer fixtures остаются следующим шагом. Не использовать число строк в factory как proxy runtime cost.
