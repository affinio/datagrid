...
- **DoD:** zero-width 10k column contract и algorithm benchmark покрыты; новый transition contract проверяет bounded window после hide, resize и reorder/pin на 10k columns; browser mounted-cell counts и real visibility/resize/pin acceptance остаются отдельными validation cases.
...
- **Исправление B:** design proposal подготовлен: `docs/internal/plans/datagrid-pivot-sparse-storage-design-2026-09-14.ru.md` фиксирует sparse storage, typed missing/null semantics, bounded dense compatibility reads, export/drilldown boundaries и memory invariants. Runtime implementation не начата до validation и согласования public accessor/export boundary.
- **Проверки:** focused pivot runtime contract проверяет guard на 2-row × 2-column output; stage regression проверяет переход valid→exceeded с сохранением последнего snapshot и typed diagnostics; pivot high-cardinality stress benchmark `bench:datagrid:pivot:stress:50k` выполнен на 50k rows, cardinality 30, 3 value specs и 3 seeds; core type-check/build проходят.
...
- **Исправление:** выполнено для горизонтальной оси: core internal helpers `resolveFirstColumnIndexAfterOffset`, `resolveLastColumnIndexBeforeOffset` и prefix-варианты стали каноническими lower-bound операциями; core column sizing и Vue app viewport используют их, при этом Vue сохраняет DOM sampling, refs, scheduling, retention и собственные boundary semantics.
- **Проверки:** core horizontal geometry contract — 4 tests passed; Vue viewport contract — 47 tests passed; core и Vue type-check проходят. Проверены пустые/граничные offsets, variable widths и prefix metrics; pinned widths остаются ответственностью core range calculation.
- **DoD:** горизонтальная offset-to-index формула имеет single internal owner; вертикальная geometry, overscan/materialization и DOM scheduling остаются отдельными владельцами и требуют следующих differential/browser slices. Desktop/touch behavior не менялся.
...
- **Исправление B:** measurement sub-slice выполнен: worker protocol artifact сохраняет command/update payload bytes (min/mean/p95/p99), roundtrip/ack lag и aggregate `maxInflightCommands` для correctness и throughput strategies; raw per-run данные остаются в том же JSON. Delta payload для unchanged window/metadata, viewport priority и backpressure не вводились. Transferables полезны для подходящего columnar payload, а не автоматически для любых object rows.
- **DoD:** host exception, uncloneable row reply и worker termination error покрыты worker regression tests; при неудаче structured clone отправляется metadata-only terminal update с тем же `requestId`; lost response покрыт transport timeout contract (`inflight` возвращается к `0`, `timedOut` увеличивается). Slow worker + rapid scroll/patch bursts остаются отдельными validation cases.
...
- **Исправление B, compute backpressure sub-slice:** postMessage compute transport ограничивает unacknowledged requests до `32`; после cap возвращает `handled:false`, и существующий core runtime выполняет synchronous fallback. Contract фиксирует 40 requests, 32 отправленных и 8 fallback без роста inflight. Row-model command protocol не изменён. Payload delta и browser pressure остаются отдельными validation cases.
...
- **Исправление A:** выполнено: `enforceLimit` и `rangeCache.evictChunks` собирают eviction candidates одним проходом за вызов и удаляют их пакетно, устраняя повторные scans protected prefix и LRU chunks; loading chunks и поведение при полностью protected cache сохранены. Изменение политики overflow не вводилось.
- **Измерение:** datasource churn baseline на 220k rows: p95 scroll-burst 0.630ms при cache limit 4096; pressure profile с limit 256 — 0.858ms, без blank viewport и с cache hit ratio 1.0. После batched range-cache eviction короткий profile на 10k rows / 20 scroll iterations показал p95 scroll-burst 2.125ms, p95 filter-burst 1.147ms, cache hit ratio 1.0; artifact сохранён в `/tmp/datagrid-range-cache-pressure.json`.
...
- **Исправление B:** design proposal подготовлен: `docs/internal/plans/datagrid-server-hierarchical-cache-design-2026-09-14.ru.md` фиксирует store identity, revision ownership, lifecycle, общие budgets, protected viewport и stale-reply invariants. Runtime implementation не начата до согласования protocol/API boundary. AG Grid описывает root cache и cache для каждого уровня grouping. [SSRM Configuration](https://www.ag-grid.com/javascript-data-grid/server-side-model-configuration/).
...
- **Исправление B:** design proposal подготовлен: `docs/internal/plans/datagrid-lazy-runtime-initialization-design-2026-09-14.ru.md` требует consumer-specific tree-shaken fixtures, parse/evaluate/create-dispose/heap measurements и сохраняет synchronous public factory semantics. Первый runtime slice выполнен: column histogram создаётся лениво при первом `getColumnHistogram`, plain rows path сохраняется; dynamic import и public API не вводились.
...
- **DoD:** integration guide обновлён; focused options/diff contract — 2 tests passed; `bench:datagrid:app-inputs` на 100k rows / 100 updates показал stable `patchRows` p95 `0.041ms` против immutable `setRows` p95 `104.754ms`. Editor/selection, allocations и model disposal остаются browser workload validation.
