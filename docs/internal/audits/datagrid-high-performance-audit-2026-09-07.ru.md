...
- **Исправление A:** выполнено частично: `enforceLimit` теперь собирает eviction candidates одним проходом за вызов и удаляет их пакетно, устраняя повторный scan protected prefix; поведение при полностью protected cache сохранено. Изменение политики overflow не вводилось.
- **Измерение:** datasource churn baseline на 220k rows: p95 scroll-burst 0.630ms при cache limit 4096; pressure profile с limit 256 — 0.858ms, без blank viewport и с cache hit ratio 1.0.
