...
- **Materialization sub-slice:** runtime получил bounded `materializeRows({ mode: "sparse" | "dense", maxCells })`; dense compatibility rows достраивают отсутствующие cells значением `null`, а превышение лимита возвращает typed `output-limit-exceeded` до materialization. Focused contract проверяет sparse-to-dense correctness и fail-closed limit. Export/drilldown adapters и `maxBytes` остаются следующими slices.
