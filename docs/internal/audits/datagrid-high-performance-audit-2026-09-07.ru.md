...
- **Transfer-list API sub-slice:** transport target получил optional transfer list и `resolveTransferList({ request, message })`; default object-row protocol и envelope не меняются. Contract передаёт explicit `ArrayBuffer` transfer list и сохраняет compute stats. Использование для реальных columnar payloads и slow worker + rapid scroll/patch bursts с browser worker остаются workload checks.
