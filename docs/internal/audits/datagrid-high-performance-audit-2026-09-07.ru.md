...
- **Исправление B:** не начинать автоматически: lazy initialization и low-level kernel extraction требуют consumer-specific tree-shaken fixtures, parse/evaluate/create-dispose/heap measurements и отдельного API/design решения.
- **DoD:** artifact показывает состав и стоимость documented capability entrypoints и повторяемый startup baseline; добавлены `docs/perf/datagrid-package-footprint-baseline.json` и `bench:datagrid:package-footprint:assert` с явными growth budgets. Consumer-specific tree-shaken fixtures и lazy initialization остаются следующим design slice. Не использовать число строк в factory как proxy runtime cost.
...
- **Исправление B:** выполнен безопасный sub-slice для options: app row-model хранит structural snapshot и пропускает recreate при новой inline-ссылке с эквивалентным содержимым; deep mutation и семантически изменённые options по-прежнему пересоздают модель. Diff rows остаётся отдельным opt-in contract, а `setRows` semantics не менялись.
- **DoD:** integration recipe выполнен; `bench:datagrid:app-inputs` на 100k rows / 100 updates показал stable `patchRows` p95 `0.048ms` против immutable `setRows` p95 `102.279ms`. Focused options contract passed; editor/selection, allocations и model disposal остаются browser workload validation.
