...
- **Drilldown acceptance sub-slice:** client row model теперь отдельно покрыт sparse pivot drilldown contract: subtotal/grand-total возвращают bounded source rows и корректный `matchCount`/`truncated`; existing host materialization сохраняется. `maxBytes` остаётся следующим slice.
