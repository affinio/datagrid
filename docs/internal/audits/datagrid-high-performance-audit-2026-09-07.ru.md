...
- **Before/after measurement:** один и тот же benchmark на 100k rows, 30 iterations для unrelated и 15 для sort-key/grouped, одинаковые seeds/процесс запуска. Unrelated `p50/p95` до→после: 1 row `25.224/34.710ms` → `0.0074/0.0530ms` (`~3400x/~655x`), 100 rows `21.521/28.346ms` → `0.182/0.222ms` (`~119x/~128x`), 1000 rows `22.916/24.240ms` → `1.376/2.607ms` (`~16.7x/~9.3x`). Sort-key остался в пределах шума: 1 row `19.078/23.544` → `19.396/22.450ms`, 100 `16.940/19.836` → `17.905/20.554ms`, 1000 `18.821/23.977` → `19.676/23.174ms`; grouped p50/p95: 1 row `66.833/89.786` → `69.046/95.398ms`, 100 `75.930/86.950` → `69.124/95.256ms`, 1000 `73.084/101.313` → `68.915/88.722ms`. Это локальный before/after model measurement без DOM/paint; sort-key сохраняет полный reorder fallback.
...
- В фактическом CI-профиле `bench:regression` теперь blocking включены interaction, browser-resource, virtualization и rendering warnings. Runtime gate также требует для каждого обязательного suite хотя бы один `runs`/`sessions`/`scenarios` sample и конечное aggregate elapsed измерение; пустой или неполный artifact становится красным.
...
- **Исправление:** design proposal реализован для core mapping: `docs/internal/plans/datagrid-bounded-physical-scroll-design-2026-09-14.ru.md` фиксирует logical/physical domains и single mapping boundary; core и viewport virtualization переводят measured native extent в logical offset при превышении browser limit, identity path сохранён. Contract matrix покрывает 1M/10M rows при высотах 24/31/100 px с top/middle/last round-trip reachability. Browser last-row reachability и полное Vue integration acceptance остаются отдельным slice.
- **DoD:** core mapping, 1M/10M contract matrix и 24/31/100 px reachability invariants закрыты; browser thumb drag, keyboard End, scrollToCell, restoration и blank-gap acceptance остаются CI-only с Chromium.
...
- **Проверки:** queue contract — 3 tests passed; renderer defer contract проверяет повторный row object с тем же `rowId` и обновлённым value, чтобы renderer получает свежий row context без slot reuse; enqueue+flush 512 prioritized cells — 10,835.64 ops/s против 8,047.56 ops/s предыдущей реализации (+34.6%). Кодовый DoD opt-in defer policy закрыт; остаётся только browser differential acceptance и measured final-content latency в CI с Chromium.
...
- **Исправление B:** design proposal реализован для core runtime: `docs/internal/plans/datagrid-pivot-sparse-storage-design-2026-09-14.ru.md` фиксирует sparse storage, typed missing/null semantics и bounded dense compatibility reads. Opt-in `sparseStorage` требует `sparseOutput`, хранит aggregate values в sparse row/column maps и отдаёт их через `readCell`/bounded `materializeRows`; dense output и default row payload не меняются. Export/browser integration остаются отдельными workload checks.
- **Измерение sub-slice:** workload 10k rows с correlated 100×100 high-cardinality sparse matrix: dense 187.24 ops/s, sparse 209.33 ops/s, 1.12x; это локальный payload benchmark, не browser SLA и не доказательство полной sparse storage.
- **API sub-slice:** согласованный typed accessor `readCell({ rowKey, columnKey, valueField })` различает `missing`, `null` и `value`, не материализуя соседние cells; bootstrap delegation и sparse value/missing contract покрыты focused test. Индексы row/column теперь поддерживаются внутри runtime, поэтому accessor не сканирует projected rows/columns при каждом sparse read. Client row model теперь протягивает opt-in `sparsePivotStorage` в тот же runtime boundary.
- **Materialization sub-slice:** runtime получил bounded `materializeRows({ mode: "sparse" | "dense", maxCells })`; dense compatibility rows достраивают отсутствующие cells значением `null`, а превышение лимита возвращает typed `output-limit-exceeded` до materialization. Focused contract проверяет sparse-to-dense correctness и fail-closed limit.
- **Drilldown acceptance sub-slice:** client row model теперь отдельно покрыт sparse pivot drilldown contract: subtotal/grand-total возвращают bounded source rows и корректный `matchCount`/`truncated`; existing host materialization сохраняется.
- **Cache budget sub-slice:** internal datasource cache manager получил incremental estimated-byte accounting при `Map.set/delete/clear`; `enforceLimit` теперь одновременно соблюдает row и byte caps, сохраняя protected ranges. Eviction не сканирует payloads заново; custom estimator и byte-cap contract покрыты focused test.
...
- **DoD:** guard срабатывает до dense row materialization, sparse accessor/storage и bounded dense compatibility contracts закрыты; остаются только heap peak и browser export/read/pinned UI workload checks в CI с Chromium.
...
- **Исправление:** выполнен безопасный sub-slice: `replaceProjectionSegment` теперь делает одну копию projection и выполняет bounded manual shift/insert без промежуточных prefix/suffix arrays и без spread argument limit. Production path дополнительно убрал вторую полную копию при обновлении state group row: immutable snapshot создаётся одной заменой, затем изменяется только соответствующая позиция. Добавлен internal chunked sequence proof-of-concept с локальным range replacement. Production fast paths обновляют group index через delta по удалённому/вставленному subtree и сдвигают последующие entries без повторного прохода по flattened row suffix; array projection oracle сохраняется до differential sequence integration. Chunked proof-of-concept дополнительно использует binary search по chunk starts для indexed reads после variable-length replacements; equal-size replacement внутри одного chunk клонирует только затронутый chunk.
- **Проверки sub-slice:** 3 chunked contracts passed; boundary regression также гарантирует, что oversized delete не портит length/index после локальной replacement; production client/tree regressions — 141 tests passed. Свежий `bench:datagrid:tree:wide-branch` на 150k rows / 3 iterations после устранения двойной копии: path p50 `15.89ms`, parent p50 `13.18ms`; это local measurement, не SLA. 300k-row benchmark indexed read `13,155,998.32 ops/s`, array oracle replacement `6,620.83 ops/s`, optimized in-chunk replacement `8,045,841.74 ops/s` (`1,216x` против array oracle). Heap profile на 300k rows / 5 toggles показал peak над baseline `+2.48MB` path и `+2.33MB` parent; после dispose heap delta `-110.30/-50.81MB`, поэтому peak, а не post-dispose delta — релевантный показатель. Production chunked sequence integration выполнена для replacement span ≥4096 через внутренний chunked sequence с сохранением малого replacement fast path. Свежий run после интеграции на 150k rows / 3 iterations: path p50 `22.692ms`, parent p50 `17.648ms`; против предыдущих `26.43/20.23ms` это примерно `14.1%/12.8%` ниже, но измерение остаётся local benchmark, не SLA.
- **DoD:** закрыт кодом, differential contracts и local benchmark: correctness regression suite проходит (`149` focused core tests); production replacement использует bounded manual shift для малых spans и внутреннюю chunked sequence для replacement span ≥4096; array oracle, repeated path/parent toggles, 300k indexed replacement и heap profile покрыты. Свежий 150k wide-branch run после интеграции дал path p50 `22.692ms`, parent p50 `17.648ms`; heap peak над baseline `+2.48MB` path и `+2.33MB` parent. Осталась только browser-visible workload validation, если она нужна для release evidence.
...
- **DoD:** host exception, uncloneable row reply, worker termination, transport timeout, bounded inflight, stale burst и columnar transfer contracts закрыты worker tests; rapid scroll/patch browser matrix остаётся CI-only с Chromium.
...
- **Browser acceptance sub-slice:** добавлен Playwright regression для `/vue/worker-grid`: 200 000 строк, 32 колонки, длинные scroll jumps, проверка продвижения logical viewport, отсутствия blank vertical bands и bounded rendered rows. Исполнение требует browser runtime; в текущем ARM64 окружении Chromium Playwright не поддерживается, поэтому тест зарегистрирован и готов для CI.
- **Transfer-list API sub-slice:** transport target получил optional transfer list и `resolveTransferList({ request, message })`; default object-row protocol и envelope не меняются. Contract передаёт explicit `ArrayBuffer` transfer list и сохраняет compute stats.
- **Columnar payload sub-slice:** row-model update получил opt-in `columnarNumericFields`; host кодирует finite numeric fields в `Float64Array` + `Uint8Array` null mask, передаёт оба buffer через transfer list, а mirror восстанавливает исходные row values. Object-row fallback сохраняется для неподходящих значений и при postMessage failure; in-memory roundtrip contract подтверждает 34 worker tests.
- **Browser validation matrix:** Playwright regression для /vue/worker-grid покрывает 100 000 строк × 32 колонки и 32 rapid scroll jumps с проверкой отсутствия blank vertical bands и bounded rendered rows. bench:datagrid:worker:pressure использует тот же route как production shaped workload; после подключения columnarNumericFields worker mode реально гоняет columnar consumer через transfer-list payload во время patch/scroll pressure. Локальный browser runtime в ARM64 окружении недоступен, поэтому execution evidence остаётся CI-only.
...
- **Исправление B:** design proposal подготовлен: `docs/internal/plans/datagrid-server-hierarchical-cache-design-2026-09-14.ru.md` фиксирует store identity, revision ownership, lifecycle, общие budgets, protected viewport и stale-reply invariants. Internal `DataSourceCacheStoreRegistry` реализует signature-gated retained reuse, generation-safe invalidation и LRU eviction retained stores; row-model wiring подключает отдельные cache manager/range cache для root и normalized grouped tree contexts, сохраняет stale visible rows при переключении context и возвращает root store при reopen. Registry ограничен 8 contexts; общий retained row budget и общий estimated-byte budget теперь enforcement-ятся между stores через существующий `rowCacheLimit`/`rowCacheMaxBytes`, а eviction выполняется между batch writes и не удаляет active/protected store. Protocol/payload boundary остаётся отдельной server integration проверкой. Regression на 1000 retained touches и benchmark 10k acquire/retain/enforce операций при cap 64 занял `2.8063ms mean` и завершился ровно с 64 retained stores.
...
- **Проверки sub-slice:** datasource row-model/context contracts 81 passed; one-seed churn baseline на 220k rows: scroll-burst p95 7.128ms, filter-burst p95 3.104ms, cache hit ratio 1.000, heap delta 0.09MB. Дополнительный registry contract проверяет отдельную LRU eviction retained store; общий retained row budget равен настроенному `rowCacheLimit` и enforcement выполняется после batch writes, без обхода на каждый row. Это consistency и local churn evidence, не browser/HTTP latency.
- **DoD:** cache/context contracts, branch reopen, соседние stores, global row/byte caps и memory limits закрыты; browser/HTTP latency остаётся workload evidence в CI.
...
- **Измерение:** benchmark harness теперь параметризуется через `DATA_GRID_ROW_HEIGHT_BENCH_ROWS`, `DATA_GRID_ROW_HEIGHT_BENCH_OVERRIDES` и `DATA_GRID_ROW_HEIGHT_BENCH_UPDATES`; baseline 200k rows / 5k overrides / 64 updates: full prefix rebuild 7.05 ops/s, sparse Fenwick path 2,722.79 ops/s, 385.96×.
...
- **DoD:** offsets и inverse lookup против contract oracle, sparse mutation regression, 1M sparse contract, 1M measurement, package type-check и Playwright acceptance для 50k-row variable-height режима покрыты; остаётся только dense/1M browser trace и pinned-pane alignment в CI с Chromium.
...
- **DoD:** integration guide обновлён; focused options/diff contract проверяет серию из 10 immutable updates без recreate owned model и с сохранением identity неизменённых row nodes; `bench:datagrid:app-inputs` на 100k rows / 100 updates показал stable `patchRows` p95 `0.053ms` против immutable `setRows` p95 `140.116ms`. Кодовый DoD закрыт; editor/selection, allocations и model disposal остаются browser workload validation в CI с Chromium.
...
- **Pointer preview:** orchestration `useDataGridGlobalPointerLifecycle.ts:57` имеет default `sync`; `raf` режим и тесты уже есть. Добавлен repeatable Vitest benchmark на 1,000 pointer events с 100 simulated frame flushes: sync `79,183.95 ops/s`, rAF `72,026.14 ops/s` (`1.10x` быстрее в этом synthetic dispatch run). Это измеряет orchestration overhead и не включает DOM/layout/paint; поэтому default timing не менялся. Browser input-to-paint и selection/fill/resize latency остаются workload validation.
