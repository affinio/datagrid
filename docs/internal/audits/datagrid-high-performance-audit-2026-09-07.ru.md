...
- **DoD:** artifact показывает состав и стоимость documented capability entrypoints и повторяемый startup baseline; добавлены `docs/perf/datagrid-package-footprint-baseline.json` и `bench:datagrid:package-footprint:assert` с явными growth budgets. Consumer-specific fixtures и lazy initialization runtime остаются отдельным implementation slice. Не использовать число строк в factory как proxy runtime cost.
