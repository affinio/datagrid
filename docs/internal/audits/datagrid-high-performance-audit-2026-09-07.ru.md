...
- **DoD:** host exception покрыт worker regression test; lost response теперь покрыт transport timeout contract (`inflight` возвращается к `0`, `timedOut` увеличивается); uncloneable reply, worker termination и slow worker + rapid scroll/patch bursts остаются отдельными validation cases.
