...
- **Исправление A:** выполнено: `enforceLimit` собирает eviction candidates одним проходом за вызов, а `rangeCache.evictChunks` использует индексированный LRU min-heap вместо сортировки всех chunks при каждом overflow; loading chunks и поведение при полностью protected cache сохранены. Изменение политики overflow не вводилось.
- **Измерение:** datasource churn baseline на 220k rows: p95 scroll-burst 0.630ms при cache limit 4096; pressure profile с limit 256 — 0.858ms, без blank viewport и с cache hit ratio 1.0. После batched range-cache eviction короткий profile на 10k rows / 20 scroll iterations показал p95 scroll-burst 2.125ms, p95 filter-burst 1.147ms, cache hit ratio 1.0; artifact сохранён в `/tmp/datagrid-range-cache-pressure.json`. Свежий synthetic churn после heap перехода (50k setRow + periodic reads) сохранил cap 64/256/1024 chunks и занял 20.089/24.748/25.766ms; это локальная asymptotic check, не browser SLA.
