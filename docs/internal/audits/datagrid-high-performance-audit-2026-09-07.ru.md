...
- **DoD:** sorted-unrelated на 100k с 1/100/1000 changed rows закрыт отдельным fast path. Добавлены correctness cases для sorted-key с filtered-hidden и pagination, а benchmark получил режим `BENCH_SORTED_PATCH_MODE=sort-key`; grouping/aggregation и 300k остаются отдельными validation cases. Sorted-key сохраняет полный reorder fallback.
