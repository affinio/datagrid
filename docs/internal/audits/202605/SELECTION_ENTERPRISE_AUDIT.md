...
1. **Active cell ownership is documented, but remount/server proof remains incomplete.**
   `snapshot.ts` stores `activeCell`; `useDataGridAppSelection.ts` stores `selectionAnchor`; `useDataGridAppActiveCellViewport.ts` and `useDataGridStageFocusRuntime.ts` restore DOM focus; `useDataGridAppInteractionController.ts` starts/commits/cancels editing. This is now documented as one cross-package state-machine contract with focused active-range, focus fallback, edit-handoff coverage, browser/component placeholder materialization selection handoff, vertical remount focus restoration, right-pinned vertical remount coverage, browser/component horizontal remount coverage, browser-level editor scroll/commit coverage, browser-level server datasource placeholder replacement coverage, component-level editor remount coverage, and stage a11y state coverage. It still needs broader keyboard move coverage over server/unloaded ranges.

2. **Selection continuity across virtualization remounts is partially proven, not fully gated.**
   Logical selection uses absolute row indexes and row ids, and rendered cells are keyed by row id/column key in `DataGridTableStageCenterPane.vue`. `DataGridTableStage.vue` now preserves grid focus ownership through virtualized cell unmount/remount and restores the visible selection anchor after scroll idle. E2E coverage proves vertical scroll-out/scroll-in preserves the anchor class, overlay segment, fill handle, and keyboard focus, covers a right-pinned selected cell remounting with its overlay/fill handle after vertical virtualization, proves horizontally virtualized editable selected cells restore anchor, fill handle, and overlay after remount, proves inline editor drafts commit before an attempted virtualization scroll, proves shell placeholder-tail materialization moves selection anchor/fill affordances to the created row after inline edit commit, and proves server datasource loading-placeholder replacement keeps the active selection anchor and fill handle on the materialized row. Component coverage proves horizontally virtualized selected columns restore anchor and fill-handle affordances when remounted, inline text editor draft state remounts with the edited cell after horizontal virtualization, and placeholder materialization moves selection anchor/fill affordances to the created row after inline edit commit.
...
4. **Focus synchronization is retry-based rather than state-machine based.**
   `dataGridFocusRestore.ts` retries focus after `nextTick` and rAF, which is pragmatic. Browser and component tests now cover virtualization remounts, pinned panes, editor scroll/commit behavior, server placeholder replacement, and horizontal virtualization; broader keyboard movement over server/unloaded ranges still needs delegated-operation coverage.
...
| --- | --- | --- |
| Active cell ownership | Documented across snapshot, anchor, focus runtime, and editing with focused contracts | Need server/unloaded keyboard and delegated-operation coverage |
| Range selection | Strong core/app support with grouped/tree, pinned, horizontal, and server placeholder remount coverage | Need delegated server operation coverage |
...
| Virtualization remount continuity | Logical model is suitable; vertical, right-pinned vertical, horizontal selected-cell remounts, inline editor scroll/commit behavior, shell placeholder materialization, and server datasource placeholder replacement are browser-covered; horizontal selected-column remount, editor draft remount, and placeholder materialization selection handoff are component-covered | Watch future server/unloaded operation semantics |
...
| Focus synchronization | Pragmatic focus restore exists with active/focus/edit handoff coverage across pinned, horizontal, editor, and server placeholder browser paths | Need server/unloaded keyboard movement coverage |
...
- Active/focus/edit ownership is specified as one state machine, including browser-level server datasource placeholder replacement proof.
...
- Browser/e2e coverage now proves vertical selection remount focus continuity, right-pinned vertical remount continuity, horizontal selected-cell remount continuity, inline editor scroll/commit behavior, shell placeholder materialization handoff, server datasource placeholder replacement, and grouped/tree selection workflows, while component coverage proves horizontal selected-column remount affordances, inline editor draft remount, and placeholder materialization selection handoff.
...
2. Execute the real-device touch selection matrix and tune hardware thresholds if the Chromium budgets do not match device traces.
3. Add server-delegated operation latency gates after copy/export, cut, clear/delete, paste, range move, and summary handlers are implemented.
