...
- 2026-05-26: Prototype bugfix: the shared vertical shell now owns an explicit scroll spacer and keeps the body pane layers sticky while syncing the center viewport `scrollTop` from the shell. This restores visible vertical scrolling under `dgPinnedNativeScroll=1` and keeps pinned chrome canvases anchored instead of scrolling out with the pane content.
