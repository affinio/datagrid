...
- Existing performance gates cover fill and enterprise copy-paste-fill benchmark phases. `bench:datagrid:enterprise:clipboard:assert` hard-fails materialized copy creation, TSV parser cost, paste payload creation, paste patch application, and total paste latency regressions. `bench:datagrid:enterprise:clipboard:browser:assert` hard-fails Chromium clipboard write/read/round-trip latency regressions with granted permissions.
...
- Pending copied and cut clipboard outlines are covered at the app range/viewport-offset level and by Playwright for vertical scroll-out/scroll-in, horizontal virtualization, and right-pinned pane remounts.
...
- Pending copied and cut cell clipboard outlines reappear after vertical/horizontal virtualization remount and right-pinned pane scroll.
...
- Paste parse time for CSV payloads and larger TSV payloads beyond the smoke assert shape.
...
- Browser clipboard failure rate and larger-payload latency beyond the focused Chromium assert shape.
