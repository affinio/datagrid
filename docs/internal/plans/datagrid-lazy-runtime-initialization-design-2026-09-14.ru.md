...
2. Выполнено для column histogram, pivot projection и aggregation engine: capability создаются через internal lazy slots при первом требующем вызове, parity покрыта focused tests; formula/tree/aggregation остаются следующими кандидатами после runtime measurements.
