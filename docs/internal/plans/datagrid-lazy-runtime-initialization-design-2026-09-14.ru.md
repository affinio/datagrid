...
2. Первый slice выполнен для column histogram: capability создаётся через internal lazy slot при первом чтении, parity покрыта focused client-row-model test; formula/pivot остаются следующими кандидатами после runtime measurements.
