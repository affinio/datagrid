...
Статус: implemented for documented capability slots; browser evidence и consumer-specific bundle validation остаются открытыми.
...
2. Выполнено для column histogram, pivot projection, aggregation engine, tree projection и formula diagnostics: capability создаются через internal lazy slots при первом требующем вызове, parity покрыта focused tests. Computed registry lazy proxy внедрён с сохранением synchronous public factory semantics; execution composition остаётся synchronous после activation.
