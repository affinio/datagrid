...
| --- | --- | --- | --- |
| P0 | Server datasource enterprise gaps: websocket/SSE transport, offline mutation replay, server grouping/tree/pivot projection, and unloaded-row operation handlers. | Polling change feed, retry/backoff, cache invalidation, placeholders, fill/edit/history, and server-client extraction are implemented; enterprise projection/live/offline work remains. | [server-data-source-checklist.md](./checklists/server-data-source-checklist.md) |
