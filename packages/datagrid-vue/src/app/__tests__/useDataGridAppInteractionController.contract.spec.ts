...
describe("useDataGridAppInteractionController contract", () => {
  ...
  it("emits a viewport scroll event when drag selection autoscrolls programmatically", () => {
    ...
    const { controller, row, bodyViewport, syncViewport } = createControllerHarness({
      rowCount: 100,
    ...
    bodyViewport.addEventListener("scroll", scrollListener)
    ...
    try {
      ...
      if (!runFrame) {
        throw new Error("Expected drag autoscroll to schedule a frame")
      }
      runFrame(0)
