<template>
  <div class="w-72 space-y-3 rounded-md border border-neutral-200 bg-white p-3 text-[11px] text-neutral-800 shadow-xl ring-1 ring-black/5 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100">
    <div class="space-y-1">
      <UiMenu ref="actionsMenuRef" :options="{ openDelay: 0, closeDelay: 0 }">
        <div class="space-y-1">
          <UiMenuItem as-child :disabled="!isSortable" @select="() => onSort('asc')">
            <button
              type="button"
          ...
          <UiMenuItem as-child :disabled="!isSortable" @select="() => onSort('desc')">
            <button
              type="button"
              class="flex w-full items-center justify-between rounded px-2 py-1.5 text-left text-[11px] font-medium transition hover:bg-blue-50 dark:hover:bg-neutral-800"
          ...
          <UiMenuItem as-child :disabled="!isGroupable" @select="handleGroupToggle">
            <button
              type="button"
              class="flex w-full items-center justify-between rounded px-2 py-1.5 text-left text-[11px] font-medium transition hover:bg-blue-50 dark:hover:bg-neutral-800"
          ...
          <UiSubMenu id="filter-popover-pin-submenu" :options="{ openDelay: 0, closeDelay: 0 }">
            <UiSubMenuTrigger as-child>
              <button
                type="button"
                class="flex w-full items-center justify-between rounded px-2 py-1.5 text-left text-[11px] font-medium transition hover:bg-blue-50 dark:hover:bg-neutral-800"
              >
            ...
            <UiSubMenuContent
              teleport-to="body"
              class="w-44 rounded-md border border-neutral-200 bg-white p-1 text-[11px] text-neutral-800 shadow-xl ring-1 ring-black/5 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100"
            >
              <UiMenuItem as-child @select="() => handlePin('left')">
                <button
                  type="button"
              ...
              <UiMenuItem as-child @select="() => handlePin('right')">
                <button
                  type="button"
                  class="flex w-full items-center justify-between rounded px-2 py-1 text-left text-[11px] transition hover:bg-blue-50 dark:hover:bg-neutral-800"
              ...
              <UiMenuItem as-child @select="() => handlePin('none')">
                <button
                  type="button"
                  class="flex w-full items-center justify-between rounded px-2 py-1 text-left text-[11px] transition hover:bg-blue-50 dark:hover:bg-neutral-800"
...
import type { VirtualListExposedMethods } from "./ui/VirtualList.types"
import type { FilterCondition, FilterOption } from "../composables/useTableFilters"
import LoadingSpinner from "./ui/LoadingSpinner.vue"
import { UiMenu, UiMenuItem, UiSubMenu, UiSubMenuContent, UiSubMenuTrigger } from "@affino/menu-vue"
...
onMounted(() => {
  queueSearch(props.search, true)
  nextTick(() => {
    actionsMenuRef.value?.controller?.open("programmatic")
  })
})

onBeforeUnmount(() => {
  clearPendingSearchTimeout()
  actionsMenuRef.value?.controller?.close("programmatic")
