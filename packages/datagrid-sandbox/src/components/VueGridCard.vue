<template>
  <article class="card">
      ...
      <div class="controls">
        ...
        <label v-if="props.mode === 'pivot' && pivotViewMode === 'pivot'">
          ...
          <select v-model="pivotLayout">
            <option value="region-status-amount">Region × Status (Amount Σ)</option>
            <option value="status-region-count">Status × Region (Count)</option>
            <option value="category-region-qty">Category × Region (Qty Σ)</option>
          </select>
        </label>
    ...
    <section class="grid-stage" :class="{ 'grid-stage--auto-row-height': props.mode === 'base' && rowHeightMode === 'auto' }">
          ...
          >
            ...
            <div class="grid-main-track" :style="mainTrackStyle">
              <div
                ...
                tabindex="0"
                @mousedown.prevent.stop="handleCellMouseDown($event, row, rowOffset, columnIndex)"
                @keydown.stop="handleCellKeydown($event, row, rowOffset, columnIndex)"
                @dblclick.stop="startInlineEdit(row, column.key)"
              >
                <button
                  v-if="props.mode === 'base' && isFillHandleCell(rowOffset, columnIndex) && !isEditingCell(row, column.key)"
                  type="button"
...
} from "@affino/datagrid-vue/worker"
import {
  buildVueColumns,
  buildVueRows,
  COLUMN_MODE_OPTIONS,
...
const pivotLayout = ref<"region-status-amount" | "status-region-count" | "category-region-qty">("region-status-amount")
...
const PIVOT_LAYOUTS: Record<typeof pivotLayout.value, DataGridPivotSpec> = {
...
let workerRowModel: DataGridWorkerOwnedRowModel<VueSandboxRow> | null = null
...
const selectionService: NonNullable<UseDataGridRuntimeOptions<VueSandboxRow>["services"]>["selection"] = {
  name: "selection",
...
if (props.mode === "worker") {
  ...
  workerHost = createDataGridWorkerOwnedRowModelHost<VueSandboxRow>({
    source: channel.port2,
    target: channel.port2,
    rows: toRowInputs(rows.value),
  })
  workerRowModel = createDataGridWorkerOwnedRowModel<VueSandboxRow>({
    source: channel.port1,
    target: channel.port1,
  })
}

const runtime = useDataGridRuntime<VueSandboxRow>({
  rows,
  columns,
  services: {
  ...
  clientRowModelOptions: props.mode === "tree"
    ? {
        initialTreeData: {
          mode: "path",
...
const displayRows = ref<readonly DataGridRowNode<VueSandboxRow>[]>([])
...
watch(
  [() => sortState.value?.key ?? "", () => sortState.value?.direction ?? "", columnFilterTextByKey, appliedAdvancedFilterExpression],
  applySortAndFilter,
...
watch(groupByField, nextField => {
  ...
  runtime.api.rows.setGroupBy(buildGroupBySpec(nextField))
  void nextTick(() => {
    syncViewportFromDom()
  })
}, { immediate: true })
...
const exportPivotLayout = (): void => {
  ...
  pivotAdvancedOutputText.value = JSON.stringify(runtime.api.pivot.exportLayout(), null, 2)
}
...
const exportPivotInterop = (): void => {
  ...
  pivotAdvancedOutputText.value = JSON.stringify(runtime.api.pivot.exportInterop(), null, 2)
}
...
const importPivotLayout = (): void => {
  ...
  try {
    runtime.api.pivot.importLayout(JSON.parse(raw) as Parameters<typeof runtime.api.pivot.importLayout>[0])
...
const pivotColumnCount = computed<number>(() => visibleColumns.value.filter(column => column.key.startsWith("pivot|")).length)
...
const totalRows = computed(() => {
  ...
  return runtime.api.rows.getCount()
})
...
const resolveRenderedRowKey = (row: DataGridRowNode<VueSandboxRow>, rowOffset: number): string => {
  if (row.rowId != null) {
    return String(row.rowId)
  }
  return `row-${viewportRowStart.value + rowOffset}`
}

const resolveRowHeightPx = (rowIndex: number): number => {
  const override = runtime.api.view.getRowHeightOverride(rowIndex)
  return override ?? normalizedBaseRowHeight.value
}

const rowStyle = (row: DataGridRowNode<VueSandboxRow>, rowOffset: number): Record<string, string> => {
  void rowHeightRenderRevision.value
  if (props.mode !== "base" || rowHeightMode.value !== "fixed") {
    return {}
  }
...
const isRowAutosizeProbe = (row: DataGridRowNode<VueSandboxRow>, rowOffset: number): boolean => {
...
const measureVisibleRowHeights = (): void => {
  if (props.mode !== "base" || rowHeightMode.value !== "auto") {
    return
  }
  runtime.api.view.measureRowHeight()
}
...
const onRowResizeMove = (event: MouseEvent): void => {
  ...
  if (Number.isFinite(rowIndex)) {
    runtime.api.view.setRowHeightOverride(rowIndex, nextHeight)
...
const startRowResize = (event: MouseEvent, _row: DataGridRowNode<VueSandboxRow>, rowOffset: number): void => {
  if (props.mode !== "base") {
    return
  }
  const rowIndex = viewportRowStart.value + rowOffset
...
const autosizeRow = (event: MouseEvent, row: DataGridRowNode<VueSandboxRow>, rowOffset: number): void => {
  if (props.mode !== "base") {
    return
  }
  const rowIndex = viewportRowStart.value + rowOffset
  ...
  void nextTick(() => {
    ...
    runtime.api.view.setRowHeightOverride(rowIndex, Math.max(MIN_ROW_HEIGHT, measured))
    rowHeightRenderRevision.value += 1
...
const viewportRowStart = computed<number>(() => runtime.virtualWindow.value?.rowStart ?? 0)
const viewportRowEnd = computed<number>(() => runtime.virtualWindow.value?.rowEnd ?? Math.max(0, totalRows.value - 1))

const topSpacerHeight = computed<number>(() => {
  return Math.max(0, viewportRowStart.value * normalizedBaseRowHeight.value)
})
...
const buildSelectionContext = (): GridSelectionContext<string | number> => {
  return {
    ...
    getRowIdByIndex: (rowIndex: number) => runtime.api.rows.get(rowIndex)?.rowId ?? null,
  }
}
...
const normalizeCellCoord = (coord: SandboxCellCoord): SandboxCellCoord | null => {
  ...
  return {
    rowIndex,
    columnIndex,
    rowId: runtime.api.rows.get(rowIndex)?.rowId ?? null,
  }
}
...
const applyClipboardSelectionRange = (range: DataGridCopyRange): void => {
  ...
  if (!normalized || !runtime.api.selection.hasSupport()) {
    return
  }
  ...
  const anchor = {
    ...
    rowId: runtime.api.rows.get(normalized.startRow)?.rowId ?? null,
  }
  const focus = {
    rowIndex: normalized.endRow,
    colIndex: normalized.endColumn,
    rowId: runtime.api.rows.get(normalized.endRow)?.rowId ?? null,
  }
  ...
  selectionSnapshot.value = snapshot
  runtime.api.selection.setSnapshot(snapshot)
}
...
const clipboardBridge = useDataGridClipboardBridge<DataGridRowNode<VueSandboxRow>, DataGridCopyRange>({
  copiedSelectionRange,
  lastCopiedPayload,
  resolveCopyRange: copyRangeHelpers.resolveCopyRange,
  getRowAtIndex: rowIndex => runtime.api.rows.get(rowIndex),
...
const applyClipboardEdits = (range: DataGridCopyRange, matrix: string[][]): number => {
  ...
  for (let rowIndex = normalized.startRow; rowIndex <= normalized.endRow; rowIndex += 1) {
    const row = runtime.api.rows.get(rowIndex)
  ...
  runtime.api.rows.applyEdits(Array.from(editsByRowId.entries(), ([rowId, data]) => ({ rowId, data })))
...
const applyFillPreview = (): void => {
  ...
  const appliedRows = applyClipboardEdits(previewRange, matrix)
  if (appliedRows > 0) {
    syncViewportFromDom()
  }
}
...
const resolveRowIndexById = (rowId: string | number): number => {
  const count = runtime.api.rows.getCount()
  for (let rowIndex = 0; rowIndex < count; rowIndex += 1) {
    if (runtime.api.rows.get(rowIndex)?.rowId === rowId) {
      return rowIndex
    }
  }
  return -1
...
const resolveBaseDataRows = (): VueSandboxRow[] => {
  const count = runtime.api.rows.getCount()
  const result: VueSandboxRow[] = []
  for (let rowIndex = 0; rowIndex < count; rowIndex += 1) {
    const node = runtime.api.rows.get(rowIndex)
    if (!node || node.rowId == null || node.kind === "group") {
      continue
    }
    const cloned = cloneRowData(node.data as VueSandboxRow)
...
const rangeMutationEngine = useDataGridRangeMutationEngine<
  VueSandboxRow,
  { rowId: string },
  GridRowsSnapshot<VueSandboxRow>,
  DataGridCopyRange
>({
  ...
  applySourceRows: nextRows => {
    runtime.api.rows.setData(nextRows.map((row, index) => ({
      rowId: row.rowId,
      originalIndex: index,
  ...
  resolveDisplayedCellValue: (row, columnKey) => {
    ...
    const node = rowIndex >= 0 ? runtime.api.rows.get(rowIndex) : null
...
const applyCellSelectionByCoord = (
  ...
  if (props.mode !== "base" || !runtime.api.selection.hasSupport()) {
    return
  }
...
const setCellSelection = (
  row: DataGridRowNode<VueSandboxRow>,
  rowOffset: number,
  columnIndex: number,
  extend: boolean,
): void => {
  ...
  const coord: SandboxCellCoord = {
    rowIndex: viewportRowStart.value + rowOffset,
    columnIndex,
...
const isCellInPendingClipboardRange = (rowOffset: number, columnIndex: number): boolean => {
  if (props.mode !== "base" || pendingClipboardOperation.value === "none") {
    return false
  }
...
const resolveCellCoordFromElement = (element: HTMLElement | null): SandboxCellCoord | null => {
  ...
  return normalizeCellCoord({ rowIndex, columnIndex, rowId: runtime.api.rows.get(rowIndex)?.rowId ?? null })
}
...
function captureRowsSnapshot(): GridRowsSnapshot<VueSandboxRow> {
  const count = runtime.api.rows.getCount()
  const snapshotRows: Array<{ rowId: string | number; row: VueSandboxRow }> = []
  for (let rowIndex = 0; rowIndex < count; rowIndex += 1) {
    const node = runtime.api.rows.get(rowIndex)
    if (!node || node.rowId == null || node.kind === "group") {
      continue
    ...
    snapshotRows.push({
      rowId: node.rowId,
      row: cloneRowData(node.data as VueSandboxRow),
    })
  }
  return { rows: snapshotRows }
}

const intentHistory = useDataGridIntentHistory<GridRowsSnapshot<VueSandboxRow>>({
  captureSnapshot: captureRowsSnapshot,
  applySnapshot: snapshot => {
    runtime.api.rows.setData(snapshot.rows.map((entry, index) => ({
      rowId: entry.rowId,
      originalIndex: index,
...
const pointerAutoScroll = useDataGridPointerAutoScroll({
  resolveInteractionState: () => ({
    isDragSelecting: props.mode === "base" && isPointerSelectingCells.value,
    isFillDragging: props.mode === "base" && isFillDragging.value,
    isRangeMoving: props.mode === "base" && isRangeMoving.value,
  }),
  resolveRangeMovePointer: () => rangeMovePointer.value,
  resolveFillPointer: () => fillPointer.value,
  resolveDragPointer: () => dragPointer.value,
...
const historyActionRunner = useDataGridHistoryActionRunner({
  ...
  closeContextMenu: () => undefined,
  canUndo: () => intentHistory.canUndo.value,
  canRedo: () => intentHistory.canRedo.value,
...
const headerResize = useDataGridHeaderResizeOrchestration<VueSandboxRow>({
...
const isEditingCell = (row: DataGridRowNode<VueSandboxRow>, columnKey: string): boolean => {
...
const startInlineEdit = (row: DataGridRowNode<VueSandboxRow>, columnKey: string): void => {
  if (props.mode !== "base" || row.kind === "group" || row.rowId == null) {
    return
  }
  editingCell.value = {
    rowId: row.rowId,
...
const focusAfterInlineEdit = (
  ...
  const nextRowIndex = moveToNextRow
    ...
    : currentRowIndex
  const nextRowId = normalizeRowId(runtime.api.rows.get(nextRowIndex)?.rowId)
  if (nextRowId == null) {
    return
  }
  applyCellSelectionByCoord({
...
const handleWindowMouseMove = (event: MouseEvent): void => {
  if (headerResize.isColumnResizing.value) {
...
const resolveViewportRangeFromElement = (element: HTMLElement): DataGridViewportRange => {
  const total = runtime.api.rows.getCount()
  if (total <= 0) {
    return { start: 0, end: 0 }
  }
  if (props.mode === "base" && rowRenderMode.value === "pagination") {
...
const syncVisibleRows = (range: DataGridViewportRange): void => {
  displayRows.value = runtime.syncRowsInRange(range)
}
...
const scheduleViewportSync = (): void => {
  ...
  viewportSyncRafHandle = window.requestAnimationFrame(() => {
...
watch(rows, () => {
  if (props.mode === "tree" || props.mode === "pivot") {
    runtime.api.rows.expandAllGroups()
  }
  void nextTick(() => {
    syncViewportFromDom()
...
const rowIndexLabel = (row: DataGridRowNode<VueSandboxRow>, rowOffset: number): string => {
  if (row.kind === "group") {
    return ""
  }
  ...
  return String(viewportRowStart.value + rowOffset + 1)
}
...
const readCell = (row: DataGridRowNode<VueSandboxRow>, key: string): string => {
  if (row.kind === "group") {
    const level = Math.max(0, row.groupMeta?.level ?? 0)
    const indent = "· ".repeat(level)
    const isGroupLabelColumn = props.mode === "tree"
      ? key === "name"
      : key === firstColumnKey.value
...
const rowClass = (row: DataGridRowNode<VueSandboxRow>): string => {
  if (props.mode === "pivot") {
...
const toggleGroupRow = (row: DataGridRowNode<VueSandboxRow>): void => {
  if (row.kind !== "group") {
    return
  }
  ...
  if (row.state.expanded) {
    runtime.api.rows.collapseGroup(groupKey)
    return
  }
  runtime.api.rows.expandGroup(groupKey)
}
...
onBeforeUnmount(() => {
  if (viewportSyncRafHandle != null) {
    window.cancelAnimationFrame(viewportSyncRafHandle)
    viewportSyncRafHandle = null
  }
