...
describe("DataGridTableStage contract", () => {
  it("mounts the shared vertical scroll shell as the main body scroll owner", async () => {
    ...
    const centerChromeLayer = sharedShell.find(".grid-body-content > .grid-chrome-layer--body-center")
  ...
  it("renders overlay segments for additive single-cell selections", () => {
    ...
    expect(centerSegments).toHaveLength(1)
    expect(centerSegments[0]?.attributes("style")).toContain("border-top-width: var(--datagrid-selection-stroke-width);")
  ...
  it("does not translate horizontal header touch panning into the center horizontal owner", () => {
    ...
    const headerCell = wrapper.find('.grid-header-viewport .grid-cell--header[data-column-key="centerA"]').element
    headerCell.dispatchEvent(createTouchEvent("touchstart", { clientX: 100, clientY: 20 }))
