...
export function resolveOverscanBuckets({ available, direction }) {
    ...
    const fractions = [
        { side: "lead", frac: leadRaw - Math.floor(leadRaw) },
        { side: "trail", frac: trailRaw - Math.floor(trailRaw) },
    ];
    fractions.sort((a, b) => {
        if (Math.abs(a.frac - b.frac) < Number.EPSILON) {
            if (normalizedDirection > 0)
                return a.side === "lead" ? -1 : 1;
            if (normalizedDirection < 0)
                return a.side === "trail" ? -1 : 1;
        }
