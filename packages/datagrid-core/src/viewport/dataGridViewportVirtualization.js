...
import { FRAME_BUDGET_CONSTANTS, VERTICAL_VIRTUALIZATION_CONSTANTS, } from "./dataGridViewportConstants";
export function createDataGridViewportVirtualization(options) {
    ...
    const { totalRowCount, effectiveRowHeight, visibleCount, poolSize, totalContentHeight, startIndex, endIndex, overscanLeading, overscanTrailing, } = core;
    const { rows: { visibleRange }, } = derived;
    ...
    const visibleSnapshotBuffers = [[], [], []];
    const rowPool = [];
    const rangeRowsByIndex = new Map();
    ...
    function ensureRowPoolSize(size) {
    ...
    function nextBufferIndex(current) {
        return current === 0 ? 1 : 0;
    }
    function nextSnapshotBufferIndex(current) {
        return current >= visibleSnapshotBuffers.length - 1 ? 0 : current + 1;
    }
    function copyToSnapshot(source, size) {
    ...
    function bumpRowPoolVersion() {
        rowPoolVersion = rowPoolVersion >= Number.MAX_SAFE_INTEGER ? 0 : rowPoolVersion + 1;
        return rowPoolVersion;
    }
    function updateVisibleRows(resolveRow, resolveRowsInRange, poolSizeValue, startIndexValue) {
        ...
        const rowsInRange = typeof resolveRowsInRange === "function"
    ...
    function resetOverscan(timestamp) {
        verticalOverscanController.reset(timestamp);
    }
    function resetScrollState(timestamp) {
        verticalOverscanController.reset(timestamp);
    ...
    function applyVirtualState(result, args) {
        const { resolveRow, resolveRowsInRange, imperativeCallbacks } = args;
        const { state: virtualState, scrollTop: nextScrollTop, visibleRange: nextRange, timestamp, } = result;
        ...
        const { pool: poolSnapshot, visibleRows: visibleRowsSnapshot, version: poolVersion } = updateVisibleRows(resolveRow, resolveRowsInRange, virtualState.poolSize, virtualState.startIndex);
    ...
    function runUpdate(args) {
        const { totalRowCount: totalRows, viewportHeight: viewportHeightValue, resolvedRowHeight, zoomFactor, virtualizationEnabled: virtualizationFlag, pendingScrollTop, lastScrollTopSample, pendingScrollTopRequest, measuredScrollTopFromPending, cachedNativeScrollHeight, containerHeight, } = args;
        ...
        if (virtualizationFlag) {
            const canReuseOverscan = deltaTop === 0 &&
        ...
        const needsScrollWrite = Math.abs(lastScrollTopSample - nextScrollTop) > verticalConfig.scrollEpsilon ||
        ...
        const visibleStartIndex = Math.max(verticalState.startIndex, Math.min(verticalState.endIndex, verticalState.startIndex + Math.max(0, verticalState.overscanLeading)));
        const visibleEndIndex = Math.min(verticalState.endIndex, visibleStartIndex + Math.max(0, verticalState.visibleCount));
        return {
            state: verticalState,
            scrollTop: nextScrollTop,
    ...
    function applyPrepared(prepared, applyArgs) {
