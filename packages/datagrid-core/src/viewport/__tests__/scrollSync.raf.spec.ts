...
describe("datagrid rAF scroll sync", () => {
  ...
  it("applies the latest sampled scroll values in rAF", () => {
    ...
    expect(applySpy).toHaveBeenCalledTimes(1)
    const call = applySpy.mock.calls[0] as unknown as [unknown, ViewportSyncState] | [unknown, ViewportSyncState, ViewportSyncState]
    const nextState = (call.length === 2 ? call[1] : call[2]) as ViewportSyncState
