...
function resolveGlobalDebugDefault() {
    if (globalWindow &&
        globalWindow.__UNITLAB_TABLE_DEBUG__) {
        return true;
    }
    return false;
}
export function createDataGridViewportDiagnostics(options) {
    const { scheduler, clock, signals } = options;
    const { debugMode, fps, frameTime, droppedFrames, layoutReads, layoutWrites, syncScrollRate, heavyUpdateRate, virtualizerUpdates, virtualizerSkips, } = signals;
    const debugDefault = resolveGlobalDebugDefault();
    let runtimeOverride = debugMode.value ? debugMode.value : null;
    ...
    function scheduleFrameLoop() {
        ...
        if (!debugMode.value)
            return;
        if (rafHandle !== null || fallbackTaskId !== null)
            return;
    ...
    function updateTiming(delta) {
        ...
        const droppedFramesThisTick = delta > FRAME_DURATION_MS ? Math.max(1, Math.ceil(delta / FRAME_DURATION_MS) - 1) : 0;
    ...
    function publishPerformanceMetrics(nowTimestamp) {
        const elapsed = Math.max(1, nowTimestamp - lastPerformancePublish);
        const rate = (count) => (count <= 0 ? 0 : (count * 1000) / elapsed);
    ...
    function setDebugMode(enabled) {
        ...
        if (runtimeOverride === normalized && debugMode.value === normalized)
            return;
