...
import { createDataGridViewportSignals, } from "./dataGridViewportSignals";
import { createDataGridViewportDiagnostics } from "./dataGridViewportDiagnostics";
import { createDataGridViewportScrollIo, } from "./dataGridViewportScrollIo";
...
import { applyHorizontalViewport, prepareHorizontalViewport, } from "./dataGridViewportHorizontalUpdate";
import { resolveCanonicalPinMode } from "../columns/pinning";
import { clampHorizontalOffset, } from "./dataGridViewportHorizontalClamp";
import { createDefaultHostEnvironment, createMonotonicClock, } from "./dataGridViewportConfig";
import { createLayoutMeasurementCache, } from "./dataGridViewportLayoutCache";
import { FRAME_BUDGET_CONSTANTS, HORIZONTAL_VIRTUALIZATION_CONSTANTS, VERTICAL_VIRTUALIZATION_CONSTANTS, } from "./dataGridViewportConstants";
import { sampleBoundingRect, sampleContainerMetrics, sampleHeaderHeight, resolveDomStats, sampleVisibleRowHeights, } from "./dataGridViewportEnvironment";
import { computePinnedWidth, resolveHorizontalSizing, resolvePendingScroll, resolveViewportDimensions, shouldNotifyNearBottom, shouldUseFastPath, } from "./dataGridViewportMath";
...
export function createDataGridViewportController(options) {
    ...
    const fallbackClientRowModel = createClientRowModel({ rows: [] });
    ...
    let lastHorizontalMeta = null;
    let lastHorizontalColumnsRef = null;
    ...
    function mergeViewportRanges(left, right) {
        if (!left)
    ...
    function isRangeOutsideVisibleRows(range) {
    ...
    function normalizeAutoMeasuredHeight(height, layoutScale) {
    ...
    function syncAutoRowHeightEstimateFromCache(options = {}) {
        ...
        const lowerBound = Number.isFinite(options.preserveLowerBound)
            ? Math.max(1, options.preserveLowerBound)
            : null;
    ...
    const modelBridge = createDataGridViewportModelBridgeService({
        ...
        onInvalidate: (invalidation) => {
            ...
            if (invalidation.reason === "rows" && invalidation.scope === "content") {
                pendingContentInvalidationRange = mergeViewportRanges(pendingContentInvalidationRange, invalidation.rowRange);
            }
            else {
    ...
    function captureLayoutMetrics(label) {
        ...
        return (Math.abs(cachedContainerHeight - previousContainerHeight) > 0.5 ||
    ...
    function emitImperativeWindow(snapshot, timestamp) {
    ...
    function getRowCountFromModel() {
        return modelBridge.getRowCount();
    }
    function resolveRowFromModel(index) {
        return modelBridge.getRow(index);
    }
    function resolveRowsInRangeFromModel(range) {
        return modelBridge.getRowsInRange(range);
    }
    function setRowModelValue(model) {
        pendingContentInvalidationRange = null;
        modelBridge.setRowModel(model);
    }
    function materializeColumnsFromModel() {
        return modelBridge.materializeColumns();
    }
    function setColumnModelValue(model) {
        pendingContentInvalidationRange = null;
    ...
    function applyColumnSnapshot(meta, start, end, payload) {
        ...
        const rangeProjectionChanged = columnSnapshot.scrollableStart !== previousScrollableStart ||
    ...
    function getViewportSyncStateValue() {
        return {
    ...
    function getVirtualWindowValue() {
    ...
    function getIntegrationSnapshotValue() {
    ...
    function maybeIngestAutoRowHeights(range, layoutScale, options = {}) {
        ...
        const measured = sampleVisibleRowHeights(hostEnvironment, recordLayoutRead, containerRef, range);
        if (!measured.length) {
            return false;
        }
        const normalized = measured
            ...
            .filter((sample) => sample.height != null)
            .map(sample => ({
            index: sample.index,
    ...
    function runUpdate(force) {
        ...
        try {
            ...
            const shouldForceAutoRowHeightMeasure = rowHeightMode === "auto" &&
            ...
            const shouldFastPath = columnWindowReady &&
            ...
            const horizontalStructureDirty = !lastHorizontalMeta ||
            ...
            const horizontalMotionDirty = pendingScrollLeftRequest != null ||
                measuredScrollLeftFromPending ||
                deltaLeft > horizontalScrollEpsilon ||
                pendingHorizontalSettle;
            const shouldRecomputeHorizontal = !lastHorizontalMeta ||
            ...
            const shouldRecomputeHorizontalSizing = !lastHorizontalSizing ||
            ...
            const horizontalUpdateForced = horizontalStructureDirty ||
                ...
                metaVersionChanged;
            pendingHorizontalSettle = false;
            const imperativeOnColumns = typeof imperativeCallbacks.onColumns === "function"
            ...
            const horizontalPrepared = shouldRecomputeHorizontal
            ...
            const bottomStabilityThreshold = Math.max(viewportHeightValue * 0.35, (effectiveRowHeight.value || baseRowHeight) * 2);
            const isNearBottomWhileMovingDown = estimatedMaxScrollTop > 0 &&
            ...
            const autoHeightChanged = maybeIngestAutoRowHeights(virtualizationResult.visibleRange, layoutScale, isNearBottomWhileMovingDown
            ...
            if (modelRange.start !== virtualizationResult.visibleRange.start ||
