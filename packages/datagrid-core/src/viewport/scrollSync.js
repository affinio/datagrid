function isAdapter(target) {
    if (!target) {
        return false;
    }
    const candidate = target;
    return (typeof candidate.getSyncTargets === "function" &&
        typeof candidate.getAppliedState === "function");
}
function setTransform(target, value) {
    if (!target)
        return;
...
function applyTransforms(targets, state, next) {
    if (!targets) {
    ...
    const scrollChanged = shouldUpdate(state.scrollLeft, next.scrollLeft) ||
        shouldUpdate(state.scrollTop, next.scrollTop);
    const pinnedLeftChanged = scrollChanged || shouldUpdate(state.pinnedOffsetLeft, next.pinnedOffsetLeft);
    const pinnedRightChanged = scrollChanged || shouldUpdate(state.pinnedOffsetRight, next.pinnedOffsetRight);
    if (scrollChanged) {
...
export function applyViewportSyncTransforms(adapterOrTargets, argB, argC) {
    ...
    const targets = adapterOrTargets;
    const state = argB;
    const next = argC ?? argB;
    applyTransforms(targets, state, next);
}
export function resetViewportSyncTransforms(targets, state) {
