...
export interface ClientRowFormulaDiagnosticsRuntime {
  ...
  commitFormulaRowRecomputeDiagnostics: (diagnostics: DataGridFormulaRowRecomputeDiagnostics | null) => void
...
export function createClientRowFormulaDiagnosticsRuntime(
  ...
  const commitFormulaDiagnostics = (diagnostics: DataGridProjectionFormulaDiagnostics | null): void => {
    if (!diagnostics) {
  ...
  const commitFormulaComputeStageDiagnostics = (
    diagnostics: DataGridFormulaComputeStageDiagnostics | null,
  ): void => {
    if (!diagnostics) {
  ...
  const commitFormulaRowRecomputeDiagnostics = (
    diagnostics: DataGridFormulaRowRecomputeDiagnostics | null,
  ): void => {
    if (!diagnostics) {
      latestFormulaRowRecomputeDiagnostics = null
      return
    }
