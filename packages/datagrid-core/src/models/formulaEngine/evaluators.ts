...
function createFormulaArrayFilled(
  contextsCount: number,
...
function sliceTokenColumnsByIndexes(
  tokenColumns: readonly (readonly unknown[])[],
...
function evaluateVectorKernelForIndexes(
...
function createSequentialRowIndexes(contextsCount: number): number[] {
...
function compileFormulaAstVectorColumnKernel(
  root: DataGridFormulaAstNode,
  resolveIdentifierTokenIndex: (identifier: string) => number | undefined,
  ...
  if (root.kind === "call") {
    ...
    if (normalizedFunctionName === "IF") {
      const conditionKernel = compileFormulaAstVectorColumnKernel(
        root.args[0] ?? ZERO_FORMULA_NODE,
        resolveIdentifierTokenIndex,
      )
      const trueKernel = compileFormulaAstVectorColumnKernel(
        root.args[1] ?? ZERO_FORMULA_NODE,
        resolveIdentifierTokenIndex,
      )
      const falseKernel = compileFormulaAstVectorColumnKernel(
      ...
      return (contextsCount, tokenColumns) => {
        ...
        for (let index = 0; index < pairKernels.length; index += 2) {
          ...
          for (let conditionIndex = 0; conditionIndex < unresolvedIndexes.length; conditionIndex += 1) {
            ...
            if (formulaNumberIsTruthy(conditionValue)) {
              matchedIndexes.push(globalIndex)
            } else {
              nextUnresolvedIndexes.push(globalIndex)
            }
          }
    ...
    if (normalizedFunctionName === "COALESCE") {
      ...
      return (contextsCount, tokenColumns) => {
        ...
        for (const argKernel of argKernels) {
          if (!argKernel || unresolvedIndexes.length === 0) {
            continue
          }
  ...
  if (root.operator === "AND") {
    return (contextsCount, tokenColumns) => {
      ...
      for (let rightIndex = 0; rightIndex < rightIndexes.length; rightIndex += 1) {
        ...
        output[rightIndexes[rightIndex] ?? 0] = isFormulaErrorValue(right)
          ? right
          : formulaNumberIsTruthy(right) ? 1 : 0
      }
      return output
    }
  }
