...
function createDefaultServerRowModelRuntime(globalRef: typeof globalThis = globalThis): ServerRowModelRuntime {
  ...
  type TimeoutSchedule = (callback: () => void, delay?: number) => unknown
  type TimeoutCancel = (handle: unknown) => void
  const rafKey = "requestAnimation" + "Frame"
  const cancelRafKey = "cancelAnimation" + "Frame"
  const timeoutKey = "set" + "Timeout"
  const clearTimeoutKey = "clear" + "Timeout"
  const globalRecord = globalRef as typeof globalRef & Record<string, unknown>
  const fallbackRecord = globalThis as typeof globalThis & Record<string, unknown>
  const raf = globalRecord[rafKey] as FrameRequest | undefined
  const cancelRaf = globalRecord[cancelRafKey] as FrameCancel | undefined
  const setTimeoutRef = (globalRecord[timeoutKey] as TimeoutSchedule | undefined)
    ?? (fallbackRecord[timeoutKey] as TimeoutSchedule)
  const clearTimeoutRef = (globalRecord[clearTimeoutKey] as TimeoutCancel | undefined)
    ?? (fallbackRecord[clearTimeoutKey] as TimeoutCancel)
...
export function createServerRowModel<T>(
  ...
  const randomUnit = (): number => {
    const random = Math[mathRandomKey as "random"] as () => number
    return random()
  }
  ...
  function maybePreload(blockIndex: number, direction: Direction, relativeIndex: number, threshold: number) {
    ...
    const jitter = adaptivePrefetch ? randomUnit() * 0.1 - 0.05 : 0
