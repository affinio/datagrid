import { buildGroupExpansionSnapshot, buildPaginationSnapshot, cloneGroupBySpec, isSameGroupExpansionSnapshot, isSameGroupBySpec, normalizeRowNode, normalizeGroupBySpec, normalizePaginationInput, normalizeViewportRange, setGroupExpansionKey, toggleGroupExpansionKey, } from "./rowModel.js";
...
export function createServerBackedRowModel(options) {
    const { source } = options;
    const sourceSubscribe = source.subscribe;
    const resolveRowId = options.resolveRowId ?? ((row, index) => {
        const value = row?.id;
        if (typeof value === "string" || typeof value === "number") {
            return value;
        }
        throw new Error(`[DataGrid] Missing row identity for server row at index ${index}. ` +
    ...
    const emptyRangeRows = Object.freeze([]);
    let lastRangeCacheRows = emptyRangeRows;
    let inFlightViewportWarmup = null;
    let warmupToken = 0;
    let warming = false;
    const listeners = new Set();
    const rowNodeCache = new Map();
    const rowNodeCacheRevision = new Map();
    ...
    const unsubscribeSource = typeof sourceSubscribe === "function"
        ? sourceSubscribe((payload) => {
            if (disposed) {
                return;
            }
    ...
    function writeRowCache(index, row) {
        ...
        while (rowNodeCache.size > rowCacheLimit) {
            const oldest = rowNodeCache.keys().next().value;
    ...
    function normalizeSourceRange(startValue, endValue) {
        if (!Number.isFinite(startValue) || !Number.isFinite(endValue)) {
            return null;
        }
        const rawStart = Math.trunc(startValue);
        const rawEnd = Math.trunc(endValue);
        const start = Math.max(0, Math.min(rawStart, rawEnd));
        const end = Math.max(start, Math.max(rawStart, rawEnd));
        return { start, end };
    }
    function resolveSourceInvalidationRange(payload) {
        if (!payload || typeof payload !== "object") {
            return null;
        }
        const nestedRange = payload.range;
        ...
        return normalizeSourceRange(payload.start, payload.end);
    }
    ...
    function clearLastRangeCache() {
        lastRangeCacheStart = -1;
        lastRangeCacheEnd = -1;
        lastRangeCacheRevision = -1;
        lastRangeCacheRows = emptyRangeRows;
    }
    function invalidateRowCacheForSourceRange(sourceRange) {
    ...
    function invalidateCachesForSourceRange(sourceRange) {
        ...
        if (lastRangeCacheStart >= 0 && lastRangeCacheEnd >= lastRangeCacheStart) {
            ...
            const intersects = sourceRange.start <= lastSourceEnd &&
    ...
    function invalidateCachesForRange(range) {
        const rowCount = getVisibleRowCount();
        if (rowCount <= 0) {
            clearLastRangeCache();
            revision += 1;
    ...
    function buildWarmupStarts(start, end, step) {
    ...
    function warmViewportRange(range) {
        const sourceRange = toSourceRange(range);
    ...
    function toRowNode(index) {
        ...
        const rowId = resolveRowId(row, sourceIndex);
    ...
    return {
        ...
        getRowsInRange(range) {
            ...
            if (lastRangeCacheRevision === cacheRevision &&
